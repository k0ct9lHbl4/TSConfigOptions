...
noImplicitThisExampleCorrect.bind({ test: "" })();
...
function strictNullChecksIncorrect1(): { a: number } {
  return {
    a: null,
  };
}

strictNullChecksIncorrect1();

function strictNullChecksIncorrect2(): { a: number } {
  return {
    a: undefined,
  };
}

strictNullChecksIncorrect2();
...
function useUnknownInCatchVariablesCorrect() {
  try {
    ...
    if (error && typeof error === "object" && "test" in error && typeof error.test === "string") {
      console.log(error.test + "hello world");
    }
  }
}
...
function noPropertyAccessFromIndexSignatureIncorrect(record: Record<string, string>) {
  console.log(record.test);
}

noPropertyAccessFromIndexSignatureIncorrect({});

function noPropertyAccessFromIndexSignatureCorrect(record: Record<string, string>) {
